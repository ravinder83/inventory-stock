...
$sql = "SELECT id,customer_id,customer_name,sum(pending_amt) as 'pending_amt',DATE_FORMAT(date,'%M %e, %Y') as 'date' FROM `orders` where pending_amt != 0 GROUP BY customer_id ORDER BY pending_amt";
...
// $sql2 = "SELECT DISTINCT orderitems.order_id from orderitems INNER JOIN orders ON orderitems.order_id = orders.id where orders.pending_amt != 0 AND orders.customer_id = 2";
...
<h2 class="text-center mt-4">Pending Customer List</h2>
